...
import {
  BaseFlattedTreeNode,
  BaseFlattedTreeNodeWithChildren,
  BaseTreeNode,
} from "./types";
...
export const flattenTree = <T extends AnyRecord>(
  ...
  // @ts-ignore
  const treeRoot: FlattenTreeNodeRoot<BaseFlattedTreeNodeWithChildren<any>> =
    getTreeRoot();
  // depth start with zero.
  // @ts-ignore
  treeRoot.children = tree.map((node, index) => {
    // @ts-ignore
    return dig(0, node, treeRoot, [index]);
  });
